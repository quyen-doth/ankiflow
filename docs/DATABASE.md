...
| `review_state` | object | 現在の SRS 状態 (ease/interval/due_date/queue/`source`...) — nullable。2 つの writer: Anki sync (`source:'anki_sync'`) と LINE rating (`source:'builtin'`)、**precedence guard** あり (`review_events` 参照) |
...
| `kind` | string | `rating` (LINE/内部 SM-2 経由の rate) · `anki_sync` (Anki からの pull) |
