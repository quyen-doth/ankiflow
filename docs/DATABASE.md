...
| `user_content_types` | Create / Resync が使用するユーザー別フォーム設定 | ユーザーごと |
...
| Field | Type | 説明 |
|---|---|---|
| `id` | string (PK) | Built-in は `form_language` / `form_it` / `form_general`; custom は任意の Firestore ID |
| `code` | string | lowercase snake_case の routing key。Built-in seed は `language` / `it` / `general` |
| `name` | string | 表示名 |
| `description` | string | 説明 |
...
### `user_content_types` — ユーザー別フォーム設定 (**runtime source**)
...
| Field | Type | 説明 |
|---|---|---|
| `id` | string (PK) | Snapshot は `{sourceId}__{uid}`、ユーザー作成分は自動 ID |
| `user_id` | string | Firebase Auth UID。query filter と Rules ownership の基準 |
| `source_content_type_id` | string? | コピー元 `content_types` の ID。ユーザー作成分は未設定 |
| `code` | string | Workspace 内の routing key。Document ID ではなくこの値を使用し、作成後は変更不可 |
| `name` | string | 表示名 |
| `description` | string | 説明 |
| `icon` | string | Icon name |
...
| `type` | string | `text` / `textarea` / `dropdown` / `checkbox_group` / `tags` / `number` |
...
### `line_link_codes` — LINE アカウント連携用コード (サーバーのみ)

Settings で発行し、LINE webhook が消費する短期データ。Client SDK からの read/write は Security Rules の
catch-all deny で禁止し、Admin SDK を使う API ルートだけがアクセスする。

| Field | Type | 説明 |
|---|---|---|
| Document ID | string | `ANKI-` + 6 文字 (`A-HJ-NP-Z2-9`) の一意コード |
| `uid` | string | 連携先 AnkiFlow user の Firebase Auth UID |
| `expires_at` | string (ISO) | 発行から 10 分後。webhook は期限切れコードを拒否して削除 |

`POST /api/notifications/line-link` は同じ user の旧コードを batch 内で削除し、新コードを `create` する。
他 user のコードと衝突した場合は上書きせず、最大 3 回再生成する。コード利用時は webhook が
`settings/{uid}.line_user_id` の保存とコード削除を同一 batch で行う。
...
| Field | Type | 注記 |
|---|---|---|
| `line_channel_access_token` / `line_user_id` | string | legacy data。現在の LINE runtime は server env と `settings/{uid}.line_user_id` を使用 |
| `notifications_enabled` | boolean | legacy data。現在のグローバル gate は `settings/global.line_notifications_available` |

> LINE secret は `LINE_CHANNEL_ACCESS_TOKEN` / `LINE_CHANNEL_SECRET` の server env で管理する。Admin UI は
> `settings/default` の LINE credential を読み書きしない。
