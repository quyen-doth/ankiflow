...
- **Output profile selection:** Study language の primary BCP 47 subtag (`en` / `zh` / `ja` など) と同名の profile を優先し、なければ `default` を使用します。選ばれた言語 profile は `default` を **継承** し、実効 field は `[...lang.fields, ...default.fields.filter(継承対象)]` になります (詳細は `docs/02-design/DATABASE.md` の継承モデル)。`inherit` / `exclude` を持たない legacy profile は読み取り時に正規化され、移行前と同じ field 集合を保ちます。`include_when: 'output_vi'` の field は output language が Vietnamese の場合だけ生成 schema と Preview / History の表示対象に含めます。General Knowledge は従来どおり local strategy で、この API を呼びません。
