...
| `lib/prompts/`          | Profile 未設定の legacy request/document 用プロンプト fallback                             |
...
4. 送信時、`POST /api/generate` が `user_content_types/{content_type_id}` の owner/routing/profile を server-side で検証し、`ai_output_profiles` から英語 prompt + Zod tool schema を構築して Claude で enrich (General の local strategy を除く)。Profile 未設定の legacy document/request は旧 resolver に fallback
