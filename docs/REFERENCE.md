...
| `scripts/`              | seed-firestore、migrate-user-data、migrate-user-content-types、set-admin-claim                |
...
2. `lib/create/formBlueprint.ts` が選択した document の `fields[]` から表示順、label、required、session persistence、標準 control を構築
3. ユーザーが共通 `CardForm` に入力。Built-in は既存の `FormType` / AI schema / strategy を維持し、追加 field は `dynamicFields` に含める
4. 送信時、`POST /api/generate` を呼び出す → Claude AI エージェントがコンテンツをエンリッチ (General の local strategy を除く)
5. `lib/pendingEntry.ts` 経由で `localStorage` に結果を保存 (`ankiflow_pending_result`)
6. ブラウザが `app/preview/page.tsx` にリダイレクト、pending entry を読み込みとクリア
7. ユーザーがカードを編集、画像/音声を選択、その後:
