...
Claude (AI エージェント、tool ベース) を呼び出して、ボキャブラリーまたは IT ボキャブラリー情報を生成します。Model は tool `submit_card` を呼び出すことが強制されるため、出力は常にスキーマに準拠 (zod で検証)。Model は `settings/global.ai_model` から取得 (デフォルト `claude-haiku-4-5`)。英語・中国語・日本語は専用 schema/prompt を使用し、それ以外の有効な BCP 47 言語は IPA、訳、例文、collocation などを含む汎用 schema を使用します。
