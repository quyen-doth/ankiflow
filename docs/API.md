...
Claude (AI エージェント、tool ベース) を呼び出して、built-in または custom Content Type のカード情報を生成します。Model は tool `submit_card` を呼び出すことが強制されるため、出力は常にスキーマに準拠 (zod で検証)。Model は `settings/global.ai_model` から取得 (デフォルト `claude-haiku-4-5`)。英語・中国語・日本語は専用 schema/prompt を使用し、それ以外の有効な BCP 47 言語は IPA、訳、例文、collocation などを含む汎用 schema を使用します。Built-in は従来の schema/strategy を維持し、追加 field は `dynamicFields` で context として渡します。
...
- **Body Params:**
    ```ts
    {
      ...
      "contentTypeName"?: "string"               // Custom prompt 用の表示名
    }
    ```
- **Custom Content Type:** `word` に primary field の値、`form_type` に user Content Type の `code` を送信します。`dynamicFields` が必要で、サーバーは `meaning_vi`、例文、翻訳と custom field を含む動的 schema を構築します。
