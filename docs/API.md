...
| `/api/entries/*`、`/api/anki/*`、`/api/history/*`、`/api/generate`、`/api/languages/detect`、`/api/audio/generate`、`/api/image` | **`withAuth`** (セッションクッキー) → 不足/不正時 401 | UID に基づいてデータをスコープ |
...
| `/api/cron/*` | ヘッダー `Authorization: Bearer` **+ `CRON_SECRET`** (constant-time 比較) → 401 | Vercel Cron 専用 |
...
| `LINE_CHANNEL_ACCESS_TOKEN` / `LINE_USER_ID` | LINE Messaging API    | `/api/notifications/send`、`/api/cron/srs-push`                   |
| `INTEGRATION_TOKEN`              | 外部システム認証トークン           | `/api/integrations/term-drafts`                                   |
| `INTEGRATION_TARGET_UID`         | term draft の作成先固定 uid        | `/api/integrations/term-drafts`（`/api/cron/srs-push` のフォールバック） |
| `CRON_SECRET`                    | Vercel Cron 認証（vercel.json 使用時は自動注入） | `/api/cron/srs-push`                             |
| `SRS_PUSH_TARGET_UID`            | SRS push 対象 uid（未設定時 `INTEGRATION_TARGET_UID` にフォールバック） | `/api/cron/srs-push`              |
...
| **GET**           | `/api/cron/srs-push`           | Vercel Cron 用 — 期限の来た SRS entry を LINE Flex Message でプッシュ (1 固定 uid スコープ)      |
...
Claude (AI エージェント、tool ベース) を呼び出して、ボキャブラリーまたは IT ボキャブラリー情報を生成します。Model は tool `submit_card` を呼び出すことが強制されるため、出力は常にスキーマに準拠 (zod で検証)。Model は `settings/global.ai_model` から取得 (デフォルト `claude-haiku-4-5`)。英語・中国語・日本語は専用 schema/prompt を使用し、それ以外の有効な BCP 47 言語は IPA、ベトナム語訳、例文、collocation などを含む汎用 schema を使用します。
...
### 6.7 Integrations & Cron（セッションクッキーなし）
...
Vercel Cron (`vercel.json`) が毎日呼び出し、期限の来た SRS entry を LINE Flex Message でプッシュ。
`scripts/send-notifications.ts` + `.github/workflows/notify.yml` (旧、`user_id` フィルタなしでクロス
ユーザー漏洩していた) を置き換え — schedule は無効化済み (`workflow_dispatch` のみ残す)。

- **認証:** ヘッダー `Authorization: Bearer <CRON_SECRET>`。
- **対象:** `env SRS_PUSH_TARGET_UID`（未設定時は `INTEGRATION_TARGET_UID` にフォールバック）。
- **クエリ:** `entries` を `user_id == target` かつ `status in ['synced','reviewed']` で取得 → due
  entries (`review_state.due_date <= now`、メモリ内フィルタ) を `lib/srs/prioritize.ts`
  (`pickDueForReview`) で優先度付け (relearning 優先 → lapses 高 → ease 低)、最大 5 件選択。
- **Response (200 OK):** `{ "pushed": 5, "words": [...] }` または期限なし時 `{ "pushed": 0 }`。
- **エラー:** トークン不正 → 401；LINE 認証情報/対象 uid 未設定 → 500；LINE push 失敗 → 502。
