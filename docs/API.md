...
Claude (AI エージェント、tool ベース) を呼び出して、built-in または custom Content Type のカード情報を生成します。Model は tool `submit_card` を呼び出すことが強制され、output profile から構築した Zod schema で検証されます。Model は `settings/global.ai_model` から取得 (デフォルト `claude-haiku-4-5`)。Create は選択中の workspace snapshot ID を `content_type_id` として送り、server が `user_content_types/{id}` の owner・routing code・`ai_output_profiles` を検証してから data-driven prompt engine を使用します。ID がない旧 request、または profile 未設定の legacy document は旧 resolver に fallback します。
...
- **Body Params:**
    ```ts
    {
      ...
      "contentTypeName"?: "string",              // Legacy custom prompt 用 hint。ID 指定時は保存済み name を優先
      "content_type_id"?: "string"               // 選択した user_content_types document ID
    }
    ```
- **Authoritative Content Type:** `content_type_id` がある場合、server は document が同じ UID に属し、保存済み `code` が `form_type` と同じ runtime route に解決されることを確認します。不一致/不正設定は 400、他 user または存在しない ID は 404。Client が instruction/schema を送ることはできません。
- **Custom Content Type:** `word` に primary field の値、`form_type` に user Content Type の `code`、`dynamicFields` にフォーム context を送信します。保存済み profile がある document は宣言された field/schema を使用します。Profile がない legacy document/request は従来の generic dynamic schema に fallback します。
- **Output profile selection:** Study language の primary BCP 47 subtag (`en` / `zh` / `ja` など) と同名の profile を優先し、なければ `default` を使用します。`include_when: 'output_vi'` の field は output language が Vietnamese の場合だけ含めます。General Knowledge は従来どおり local strategy で、この API を呼びません。
...
`.github/workflows/srs-push.yml` が毎時呼び出し、通知を有効にした user へ期限の来た SRS entry を
LINE Flex Message で fan-out する。`vercel.json` からは cron 設定を削除済み。
...
- **時刻・idempotency:** `line_timezone` (不正/未設定は UTC) の現在時刻が `line_schedule_hours` に含まれる場合のみ送信。同一 user・同一ローカル日時/時は `line_last_push_key` で再送を抑止し、push 成功後だけ key を更新する。
