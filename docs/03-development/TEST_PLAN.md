...
| 種別 | 手段 | 対象 | 件数 |
| --- | --- | --- | --- |
| 単体試験 | Vitest (jsdom) | ロジック、API ルート、データ変換、移行スクリプト | 115 ファイル |
...
単体試験と検証仕様は同一の実行系で動作する。`npm run verify` が実行するのは `verify/unit/` の 115 ファイルと、検証仕様をまとめて評価する `verify/matrix.test.ts` の 1 ファイルであり、実測は **116 ファイル / 1,395 件、全件成功**である。

検証仕様のファイル (`verify/specs/*.verify.tsx`) は、それ自体が試験ファイルとして実行されるのではなく、`matrix.test.ts` から登録・評価される。したがって 115 と 72 を足し合わせることはできない。
...
| TC-051 | FR-051 | 履歴の詳細を表示できること | `verify/unit/history-page`、`verify/unit/history-detail-language-pair`、`verify/unit/entry-custom-fields` |
| — | FR-052 | 履歴からの再作成 | **試験なし。FR-052 は未実装である** |
...
| TC-071 | FR-071 | 新規利用者向け既定データを編集できること | `verify/unit/admin-default-sync`、`verify/unit/admin-page` |
...
| エージェント指針 | `AGENTS.md` と `CLAUDE.md` の共有本文が一致すること | `verify/unit/agent-instructions-sync` |
...
| プレビューと編集 | 5 | 5 | 全件 |
| Anki 連携 | 5 | 5 | 全件 |
| 履歴とダッシュボード | 5 | 4 | FR-052 を除く全件 |
| マスターデータ管理 | 6 | 6 | 全件 |
| 管理者機能 | 3 | 3 | 全件 |
| 復習通知 | 4 | 4 | 全件 |

実装済みの機能要件 43 件すべてに、対応する試験ケースが存在する。未実装の FR-052 には試験がない。

ただし**対応する試験が存在することと、その要件が十分に検証されていることは同じではない**。とりわけ次の要件は、自動試験が担保する範囲が限定的である。
...
| 版数 | 日付 | 変更内容 | 変更者 |
| --- | --- | --- | --- |
| 1.3 | 2026-08-02 | エージェント指針同期 guard の追加後に Vitest 件数を再計測し、品質維持試験へ同 guard を追記 | hong-quyen |
