...
> **採用担当者の方へ** — このリポジトリは私の転職活動用ポートフォリオです。実務に近い構成（外部 API 連携・認証・データモデリング・自動テスト基盤）を一人称で設計・実装し、**コードの品質と運用を意識した開発ができること**を示すために作成しました。まずは本 README で全体像を、続いて「[技術的ハイライト](#-技術的ハイライト)」で工夫した点をご覧いただければ幸いです。
...
> 実際の画面キャプチャを `docs/screenshots/` に配置し、下のコメントを外して掲載します。

<!-- 画像を docs/screenshots/ に置いてから、この行のコメントを外してください:
| ダッシュボード | カード作成 | プレビュー |
|---|---|---|
...
転職活動中の Web アプリケーションエンジニアです。本プロジェクトでは、要件定義からアーキテクチャ設計、実装、テスト基盤の整備までを一人称で担当しました。

- **GitHub**: <!-- プロフィール URL -->
- **連絡先**: <!-- メールアドレス等 -->

ご覧いただきありがとうございます。ご質問やフィードバックがあれば、お気軽にご連絡ください。
