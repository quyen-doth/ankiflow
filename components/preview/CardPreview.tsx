    ...
    <div className="flex flex-col gap-4" {...verifyAttrs({ unit: 'CardPreview', tab: activeTab, flipped })}>
      ...
      >
        ...
        >
          {/* Front */}
          <div
            className="bg-white rounded-xl shadow-card border border-outline-var/30 p-6 min-h-36 flex flex-col items-center justify-center gap-2"
          ...
          >
            ...
            {(audioUrl || entry.audio_url) && (
              <button
                ...
                onClick={toggleAudio}
                className={cn(
                  'flex items-center gap-1.5 text-label-sm px-3 py-1 rounded-full border transition-colors mt-2',
                  isPlaying
                    ? 'border-primary text-primary bg-primary/5'
                    : 'border-outline-var/50 text-on-surface-var hover:border-primary hover:text-primary'
                )}
              >
          ...
          {/* Back */}
          <div
            className="absolute inset-0 bg-surface-low rounded-xl border border-outline-var/30 p-6 min-h-36 flex flex-col items-center justify-center gap-2"
            style={{
      ...
      {/* Hint */}
      <p className="text-label-sm text-on-surface-var text-center">Click card to flip</p>
    </div>
  )
}
