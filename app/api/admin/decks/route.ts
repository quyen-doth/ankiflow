...
import { parseBody, DeckPostSchema, DeckPutSchema } from '@/lib/validation'
...
async function POST_handler(request: NextRequest) {
  try {
    const parsed = parseBody(DeckPostSchema, await request.json())
...
async function PUT_handler(request: NextRequest) {
  try {
    const parsed = parseBody(DeckPutSchema, await request.json())
    if (!parsed.ok) return parsed.response
